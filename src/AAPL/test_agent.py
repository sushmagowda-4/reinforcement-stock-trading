...
def main():
    # 1. Dataset path (Using raw string to prevent backslash escaping errors)
    file_path = r"D:\RL_project\APPL\AAPL_60min_2023_2025.csv"
    df, feature_cols = load_and_preprocess_data(file_path)

    # Use entire dataset or test slice
    test_df = df.copy()

    # Environment parameters (aligned with training env setup)
    ...
    # 2. Build test environment with saved VecNormalize statistics
    ...
    model_path = "model_aapl_best"  # Or "model_aapl_best" if updated
    ...
    plt.plot(equity_curve, label="AAPL Equity", color="tab:blue")
    plt.title("Out-of-Sample Equity Curve Evaluation (2023–2025)")
    plt.xlabel("Steps")
    plt.ylabel("Equity ($)")
