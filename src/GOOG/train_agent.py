...
TICKER = "GOOG"
DATA_PATH = r"D:/RL_project/GOOG/GOOG_60min_2017_2022.csv"
...
# Centralized environment configurations to keep train & eval synchronized
ENV_KWARGS = dict(
    window_size=WIN,
    sl_options=SL_OPTS,
    tp_options=TP_OPTS,
    spread_pips=0.01,         # Low spread friction for stocks
    commission_pips=0.001,
    max_slippage_pips=0.01,
    hold_reward_weight=0.001, # Small weight to deter unnecessary waiting
...
def evaluate_model(model: PPO, eval_env: VecNormalize, deterministic: bool = True):
    ...
    while True:
        ...
        # Retrieve equity from info dict before auto-reset on episode completion
...
def make_env_factory(df, feature_cols, random_start=False, min_steps=500, max_steps=None):
    """Creates a zero-argument environment constructor for DummyVecEnv."""
    def _init():
...
def main():
    ...
    # 2. Build Training Vectorized Environment with Normalization
    ...
    # 3. Setup Checkpoint and Output Directories
    ...
    # 6. Build Deterministic Evaluation Environments
    ...
    # 7. Select Best Model Based on Out-of-Sample Final Equity
    ...
    # Individual Out-of-Sample Equity Plot
    plt.figure(figsize=(12, 6))
